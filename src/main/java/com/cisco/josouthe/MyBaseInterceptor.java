...
public abstract class MyBaseInterceptor extends AGenericInterceptor {
    ...
    protected double apigeeVersion;

    public MyBaseInterceptor() {
        super();
        initialize();
        String apigeeVersionString = System.getenv("APIGEE_RELEASE"); //ie: "4.50.00"
        getLogger().info("Initializing agent for Apigee Version: "+ apigeeVersionString);
        if("".equals(apigeeVersionString)) {
            getLogger().info("Unable to determine Apigee Version, environment variable missing? APIGEE_RELEASE");
            apigeeVersion = 0.0;
        } else {
            apigeeVersion = Double.parseDouble(apigeeVersionString.replaceAll("^(\\d+\\.\\d+).*", "$1"));
        }
    }
